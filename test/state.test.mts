...
import { clearState, type IssueState, readState, STATE_DIR, takeStop, writeState } from "../src/state.mts";
...
test("clearState drops the file and accepts a missing one", async () => {
  const target = await repo();
  await writeState(target, STATE);
  await clearState(target, 7);
  assert.equal(await readState(target, 7), undefined);
  await clearState(target, 7);
});
